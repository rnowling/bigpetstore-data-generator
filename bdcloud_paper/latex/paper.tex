...
where $a_i$ is the amount of item category $i$ ``stuff'' remaining at time $t$. The variable $u_i$ gives the average usage rate and $\sigma^2_i$ gives the variance of the usage rate for item category $i$. The exhaustion time $ T_{E,i}$ for each item category is found from the usage simulations.
...
Markov Models are used to model the customer's purchasing behavior for each item category.  Each state corresponds to an item in that category. The transition probabilities are defined so-as to model the customer's buying preferences.  
...
The item category HMM could be extended to consider additional factors such as the amount of money spent so far in the transaction and the customer's spending habits.
