...
For each proposed customer location zip code $z$, we compute the distance between $z$ and every store's zip code to find the closest store.  We compute the distance $d$ between $z$ and the zip code $s_z$ of the closest store.  The weight $w_z$ for each zip code $z$ is modeled using an exponential distribution with the average distance $\beta$.
...
where $a_i$ is the amount of item category $i$ ``stuff'' remaining at time $t$. The variable $u_i$ gives the average usage rate and $\sigma^2_i$ gives the variance of the usage rate for item category $i$. The SDE is numerically integrated using the Euler-Maruyama method\cite{Klouden13} with time step $\Delta t_n$ sampled from a Poisson process:
